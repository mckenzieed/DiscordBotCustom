...
var RankObject = require('../objects/rankObject');
var rankToApConverter = require('./rankToApConverter.js');
...
var getParsesBasedOnSpec = function (url, character, boss, metric, context) {
    request({
        ...
        if (!error && response.statusCode === 200) {
            ...
            if (parses.length > 0) {
                pcHelpers.announceParses(parses, character, boss, metric, context.msg);
            }
...
var getGuildReport = function (url, guild, context) {
    request({
        url: url,
        json: true
    }, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            if (body.length === 0) {
                pcHelpers.announceGuildLogsNotFound(guild, context.msg)
            }
            else {
                var report = body[body.length-1].id;
                pcHelpers.announceGuildReport(report, guild, context.msg);
            }
        }
        else {
            var errorMessage = error || response.statusMessage;
            pcHelpers.announceGuildNotFound(guild, errorMessage, context.msg);
        }
    });
} 

var getGuildRank = function (url, guild, context) {
    request({
        url: url,
        json: true
    }, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            if (body.length === 0) {
                pcHelpers.announceGuildNotFound(guild, "Not Ranked... (should have err'd)", context.msg)
            }
            else {
                ...
                pcHelpers.announceRank(rankObject, guild, context.msg);
...
var getArtifactInfo = function (url, character, context) {
    request({
        url: url,
        json: true
    }, function (error, response, body) {
        if (!error && response.statusCode === 200) {
            ...
            var mainHandWeapon = items.mainHand;
            var weaponName = mainHandWeapon.name;
            var itemLevel = mainHandWeapon.itemLevel;
            var traits = mainHandWeapon.artifactTraits;
            var relics = mainHandWeapon.relics;
            if (!traits) {
                return "Need to err here; not an artifact";
            }
            ...
            var totalAp = rankToApConverter.convert(rank);

            pcHelpers.announceTest(totalAp, weaponName, rank, character, context.msg);
